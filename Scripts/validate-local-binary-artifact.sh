...
env \
  -u LLAMA_TEST_MODEL_PATH \
  -u CALGACUS_TEST_MODEL_PATH \
  -u CLLM_COMMON_CHAT_LIVE_MODEL_PATH \
  -u CLLM_VISION_MODEL_PATH \
  -u CLLM_VISION_MMPROJ_PATH \
  -u CLLM_VISION_CONTEXT \
  -u CLLM_AUDIO_MODEL_PATH \
  -u CLLM_AUDIO_MMPROJ_PATH \
  -u CLLM_AUDIO_SAMPLE_PATH \
  -u CLLM_AUDIO_CONTEXT \
  CARBOCATION_RUN_LIVE_TESTS=0 \
  CARBOCATION_RUN_APPLE_INTELLIGENCE_LIVE_TEST=0 \
  swift test
